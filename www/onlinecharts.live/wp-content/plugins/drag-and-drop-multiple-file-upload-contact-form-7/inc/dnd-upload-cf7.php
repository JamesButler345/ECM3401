<?php
	...
	*/
	...
	function dnd_cf7_upload_form_tag_handler( $tag ) {
		...
		return sprintf('<div id="upload-elements">
					<div id="upload-container"><h3 id="load-network-title">Load Network</h3><span class="wpcf7-form-control-wrap %1$s"><input %2$s />%3$s</span><div style="font-family: Source Serif Pro, serif; font-weight: 375; margin-bottom: 5px;"><input type="checkbox" id="ita_data"> Please tick when uploading Italian data?</input></div></div></div>',	sanitize_html_class( $tag->name ), $atts, $validation_error );
	}
